...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            var name = parts[1];
            var gradebookype = parts[2];
            var isWeighted = bool.Parse(parts[3]);

            BaseGradeBook gradeBook = null;

            if (gradebookype.ToLower() == "standard")
                gradeBook = new StandardGradeBook(name, isWeighted);
            else if (gradebookype.ToLower() == "ranked")
                gradeBook = new RankedGradeBook(name, isWeighted);
            else
            {
                Console.WriteLine($"{gradebookype} is not a supported type of gradebook, please try again");
                return;
            }
